...
@section('title', 'Reporte Equipo Computo')

@section('content_header')
    <h1>Reporte de Equipo Computo</h1>
@stop
...
@section('content')
    ...
    <div class="card">
         <div class="card-body">
            ...
            <table class="table table-bordered table-hover table-striped TB" id="ReporteEquipoComputo">
                <thead>
                    <tr>
                        <th>CODIGO</th>
                        <th>CODIGO</th>
                ...
                <tbody>
                    @foreach ($listaEquipoComputo as $listado)
                        <tr>
                            <td>{{ $listado->CODIGO_CUENTA}}</td>
                            <td>{{ $listado->CODIGO }}</th>
                            <td>{{ $listado->descripcionActivoFjo }}</th>
                            <td>{{ $listado->CatEquipoMarca }}</th>
                            <td>{{ $listado->CatequipoFechaCompra }}</th>
                            <td>{{ 'C$ ' . $listado->catEquipoCostoEquipo }}</th>
                            <td>{{ $listado->vidaUtilActivoFijo }}</td>
                            <td>{{ $listado->MESES }}</td>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            $('#ReporteEquipoComputo').DataTable({
                paginate: false,
                dom: 'Bfrtip',
                buttons: [{
                        extend: 'copy',
